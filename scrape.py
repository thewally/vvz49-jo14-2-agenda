#!/usr/bin/env python3
"""
Agenda-feed voor ST SO Soest/VVZ'49 O14-2 (JO14-2).
...
TEAM_NAME = "ST SO Soest/VVZ'49 O14-2"
UID_NAMESPACE = "vvz49-jo14-2"

# Label achter de wedstrijdtitel, bv. "[UIT] Kampong O14-5 (JO14-2)".
AGENDA_LABEL = "JO14-2"
...
def api_get(article: str, **params) -> object:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (compatible; jo14-2-agenda/1.0)"})
...
def build_ics(state: dict, activiteiten: list[dict], now: datetime, verwachte_speeldagen: list[str] | None = None) -> str:
    ...
    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//vvz49-jo14-2-agenda//NL",
        "CALSCALE:GREGORIAN",
        "X-WR-CALNAME:ST SO Soest/VVZ'49 O14-2",
    ]
    ...
    for uid, entry in sorted(state.items(), key=lambda kv: kv[1]["wedstrijddatum"]):
        ...
        # (JO14-2)" -- de eigen teamnaam staat al in de agenda-titel zelf.
